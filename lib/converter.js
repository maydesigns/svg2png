...
function convert(options) {
    ...
    page.onResourceReceived = function (response) {
      if (response.status >= 400) {
        console.error("Error during content retrieval.");
        phantom.exit()
        return;
      }
    }
    ...
    page.onResourceTimeout = function (response) {
      console.error("Timeout during content retrieval.");
      phantom.exit()
      return;
    }
    ...
    page.onLoadFinished = function (status) {
        ...
        var result = resultIdentifiers[options.format] + page.renderBase64(format);
